...
-(void)gotoToolHost:(ccTime)delta
{
    ...
    [[SimpleAudioEngine sharedEngine] playEffect:BUNDLE_FULL_PATH(@"/sfx/sfx_journey_map_general_enter_question.wav")];
...
-(void)testForNodeTouchAt:(CGPoint)lOnMap
{
    for (id go in [gw AllGameObjects]) {
        if([go conformsToProtocol:@protocol(Selectable)])
        {
            ...
            {
                [[SimpleAudioEngine sharedEngine] playEffect:BUNDLE_FULL_PATH(@"/sfx/sfx_journey_map_general_node_pin_tap.wav")];;
                break;
            }
        }
    }
...
-(void)ccTouchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (touchCount==1 && !didJustChangeZoom) {
        ...
        if(touchStartedInNodeMap)
        {
            ...
            [[SimpleAudioEngine sharedEngine] playEffect:BUNDLE_FULL_PATH(@"/sfx/sfx_journey_map_general_navigating_(panning_map).wav")];
...
//            [daemon setRestingPoint:l];
    ...
    if([touches count]>1 && !didJustChangeZoom)
    {
        ...
        {
            [[SimpleAudioEngine sharedEngine] playEffect:BUNDLE_FULL_PATH(@"/sfx/sfx_journey_map_general_zooming_map.wav")];
